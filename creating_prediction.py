...
def prepare_features(df, structure_features, af_features, existing_scalers):
    ...
    domain_indices = df['Domain'].astype('category').cat.codes.values
    domain_sequences = np.array(df['encode_domain'].tolist())
    
    # Get the domain from the dataset
    domain = df['Domain'].iloc[0]

    if domain not in existing_scalers:
        raise ValueError(f"Scalers for domain '{domain}' not found.")

    struct_scaler = existing_scalers[domain]['structure']
    alphafold_scaler = existing_scalers[domain]['alphafold']

    # Debugging
    print(f"Applying scaler for {domain}: Struct shape: {struct_features.shape}, AlphaFold shape: {alphafold_features.shape}")
    ...
    return seq_features, struct_scaled, alphafold_scaled, domain_indices, domain_sequences
...
#--------------------- Grabbing scalers ---------------------#
scaler_path = os.path.join(os.path.dirname(__file__), 'domain_scaler.joblib')
...
seq_features, struct_scaled, alphafold_scaled, domain_indices, domain_sequences = prepare_features(
